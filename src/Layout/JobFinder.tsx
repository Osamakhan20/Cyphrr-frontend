  ...
  return (
    <section className="bg-secondary px-6 py-16">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
        ...
        >
          <Job1 className="rounded-2xl w-72 md:w-80" />
          <Job2 className="absolute -bottom-6 -right-6 w-56 md:w-64 rounded-2xl border-4 border-bgCard" />
          <div className="absolute -top-4 right-10 w-12 h-12 bg-primary rounded-xl" />
        </div>

        <div className={imageFirst ? "order-2" : "order-1"}>
          <p className="text-primary text-sm mb-2">About Our Job Finder</p>

          <h2 className="text-textGray text-2xl md:text-4xl font-bold mb-4">
          ...
          <div className="flex flex-wrap gap-3">
            {jobSeekers.map((item) => (
              <span
                key={item}
                className="bg-bgCard text-primary text-sm px-4 py-2 rounded-md hover:bg-primary hover:text-black transition"
              >
                {item}
              </span>
            ))}
